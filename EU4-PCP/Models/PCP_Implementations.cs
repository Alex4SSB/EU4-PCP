...
public static class PCP_Implementations
{
    ...
    private static string GameFolder
    {
        get
        {
            ...
            {
                _gamePath = Directory.GetParent(GamePath + LOC_PATH).FullName;
            }
            return _gamePath;
        }
    }
    ...
    {
        var index = RE_PROVINCE_FILE().Match(fileName).Groups["index"].Value;
    ...
    public static string OwnerOrCulture(string fileContent, EventType scope, DateTime selectedStartDate)
    {
        ...
        if (string.IsNullOrEmpty(currentValue))
        {
            ...
            {
                EventType.Province => RE_PROVINCE_OWNER().Match(fileContent),
                EventType.Country => RE_PRIMARY_CULTURE().Match(fileContent),
    ...
    public static string LastEvent(string eFile, EventType scope, DateTime selectedStartDate)
    {
        ...
        {
            EventType.Province => RE_PROVINCE_EVENT().Matches(eFile),
            EventType.Country => RE_CULTURE_EVENT().Matches(eFile),
            _ => throw new NotImplementedException(),
        };
    ...
    public static void CountryCulSetup()
    {
        ...
        {
            var fileName = RE_COUNTRY_TAG().Match(cFile.File);
            if (!fileName.Success)
                return;
    ...
    public static void CultureSetup(string culFile, object mutex, List<Culture> cultureList)
    {
        ...
        var groups = RE_CULTURE_GROUPS().Matches(RE_CULTURE_CLEAR().Replace(text, ""));

        Parallel.ForEach(groups, group =>
        {
            var cultures = RE_CULTURE_SINGLE().Matches(group.Groups["cultures"].Value);
    ...
    {
        var name = "";
        if (RE_REMOVE_FILE_EXTENSION().Match(file.File) is Match fileName && fileName.Success)
        {
    ...
    public static Bookmark BookPrep(string bookFile)
    {
        ...
        var codeMatch = RE_BOOKMARK_CODE().Match(bFile);
        var dateMatch = RE_BOOKMARK_DATE().Match(bFile);
        ...
        return new()
        {
            ...
            Date = tempDate,
            IsDefault = RE_BOOKMARK_DEFINITION().Match(bFile).Groups["default"].Success
        };
    }
    ...
    public static void FetchDefines()
    {
        ...
        if (!Directory.Exists(SteamModPath + DEFINES_FOLDER_PATH))
            return;
        ...
        var path = SteamModPath + DEFINES_FILE_PATH;
        if (File.Exists(path))
    ...
    public static void DefinesPrep()
    {
        ...
        DefinesSetup(GamePath + DEFINES_FILE_PATH);
    }
    ...
    private static void DefinesSetup(string path, object mutex = null)
    {
        ...
        match = RE_DEFINES_DATE().Match(fileContent);
        if (!match.Success)
            return;
    ...
    public static ModObj ModPrep(string modFile, string modParentDir)
    {
        ...
        var nameMatch = RE_MOD_NAME().Match(mFile);
        var pathMatch = RE_MOD_PATH().Match(mFile);
        var verMatch = RE_MOD_VERSION().Match(mFile);
    ...
    {
        var vals = RE_MOD_REPLACE().Matches(rFile)
            .OfType<Match>()
        ...
        return new Replace
        {
            Cultures = vals.Contains(CULTURES_REP),
            Bookmarks = vals.Contains(BOOKMARKS_REP),
            ProvNames = vals.Contains(PROV_NAMES_REP),
            Countries = vals.Contains(COUNTRIES_REP),
            Provinces = vals.Contains(PROVINCES_REP),
            Localisation = vals.Contains(LOCALISATION_REP)
        };
    }
    ...
    private static bool PathHandler(Scope scope, string setting)
    {
        ...
        switch (scope)
        {
            case Scope.Game when !File.Exists(setting + GAME_FILE):
                return ErrorMsg(ErrorType.GameExe);
            case Scope.Mod:
                if (string.IsNullOrEmpty(setting))
                {
                    var path = SelectedDocsPath + MOD_PATH;
    ...
    public static void GameVer()
    {
        ...
        try
        {
            logText = File.ReadAllText(SelectedDocsPath + GAME_LOG_PATH, UTF8);
        }
        catch (Exception) { }
    ...
    public static string GameVer(string gameLog)
    {
        ...
        if (RE_GAME_VERSION().Match(gameLog) is Match match && match.Success)
    ...
    {
        var match = RE_MAX_PROVINCES().Match(fileContent);
    ...
    public static bool MaxProvinces(Scope scope)
    {
        ...
        try
        {
            d_file = File.ReadAllText(filePath + DEF_MAP_PATH, UTF8);
        }
        catch (Exception)
        {
    ...
    {
        if (!WriteProvinces(Provinces.Values.OrderBy(p => p.Index), SteamModPath + DEFIN_PATH))
    ...
    public static bool WriteDefMap(string newMax)
    {
        ...
        try
        {
            defMap = File.ReadAllText(SteamModPath + DEF_MAP_PATH);
        }
        catch (Exception)
        {
                return ErrorMsg(ErrorType.DefMapRead);
        }
        defMap = RE_DEF_MAP().Replace(defMap, $"max_provinces = {newMax}");
        try
        {
            File.WriteAllText(SteamModPath + DEF_MAP_PATH, defMap, UTF8);
        }
        catch (Exception)
        {
    ...
    public static bool LocalisationSetup(bool enBooks, bool enProvs = true)
    {
        ...
        if (SelectedMod)
        {
            ...
            if (!SelectedMod.Replace.Localisation)
            {
                ...
                dirs[1] = path + LOC_PATH;
            }

            path = SteamModPath;
        }

        dirs[0] = path + LOC_PATH;
    ...
    private static List<string> PathCombiner(params string[] dirs)
    {
        ...
        try
        {
            files = dirs.Where(d => Directory.Exists(d)).SelectMany(d => Directory.GetFiles(d, "*", SearchOption.AllDirectories).Where(f => RE_LOCALISATION_FILE().Match(f).Success)).ToList();
        }
        catch (Exception)
        {
            return null;
        ...
        if (dirs.Length > 1 && files.Any(d => d.Contains(REP_LOC_PATH)))
        {
    ...
    private static void CacheLoc(ref List<Indexer> indexList, bool enBooks)
    {
        ...
        Parallel.ForEach(indexList, item =>
        {
            ...
            var provMatches = RE_LOCALISATION_PROVINCES().Matches(text);
    ...
    {
        FileType.Country => HIST_COUNTRY_PATH,
        FileType.Bookmark => BOOKMARKS_PATH,
        FileType.Province => HIST_PROV_PATH,
        FileType.ProvName => PROV_NAMES_PATH,
        _ => throw new NotImplementedException(),
    };
    ...
    private static void Error_Msg(ErrorType type) => _ = (type switch
    {
        ...
        ErrorType.HistoryProvFolder => MessageBox.Show($"The {HIST_PROV_PATH} folder is missing or inaccessible",
